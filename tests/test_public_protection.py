...
def test_render_edge_uses_the_rightmost_forwarded_ip(client, monkeypatch):
    monkeypatch.setattr(
        "support_requests.tasks.analyze_support_request.delay_on_commit", lambda request_id: None
    )
    url = reverse("support_requests:api-create")
    options = {"content_type": "application/json", "REMOTE_ADDR": "10.0.0.10"}
    ...
    other_client = client.post(
        url,
        VALID_REQUEST,
        HTTP_X_FORWARDED_FOR="198.51.100.1, 203.0.113.21",
        **options,
    )
